...
def line_search_wolfe1(f, fprime, xk, pk, gfk=None,
    ...
    """
    #if no gradient, take the approx  gradient now
    if gfk is None:
        gfk = fprime(xk)
    
    #checks to see if fprime is a tuple
    ...
    #gval is a list containing the gradient
    gval = [gfk]
    gc = [0]
    fc = [0]

    #phi returns the objective function value (LL) with 
    #argument xk + s*pk 
    #where s is an argument its passed
    #returns new test LL
    def phi(s):
        fc[0] += 1
        return f(xk + s*pk, *args)

    #derphi returns dot product of approximated gradient 
    #at new values, and pk
    #returns new test derphi...
    def derphi(s):
    ...
    #derphi is dot product of direction and  approximated gradient
    derphi0 = np.dot(gfk, pk)

    #call SCALAR SEARCH WOLFE 1
...
def scalar_search_wolfe1(phi, derphi, phi0=None, old_phi0=None, derphi0=None,
    ...
    for i in xrange(maxiter):
        ...
        if task[:2] == b'FG' and not np.isnan(phi1):
            alpha1 = stp
            phi1 = phi(stp)
            derphi1 = derphi(stp)
        else:
...
def line_search_wolfe2(f, myfprime, xk, pk, gfk=None, old_fval=None,
    ...
    if isinstance(myfprime, tuple):
        ...
        def derphi(alpha):
            gc[0] += 1
            ###Joey Edit###
            #gval[0] = fprime(xk+float(alpha)*pk, *args)  # store for later use
